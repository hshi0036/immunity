...
class WPLE_SubAdmin extends WPLE_Admin_Page
{
    ...
    public function wple_force_https_page()
    {
        ...
        $page = "<h2>" . __( 'Force HTTPS', 'wp-letsencrypt-ssl' ) . "</h2>\r\n    <div class=\"wple-force\">\r\n      <p>" . $this->wple_kses( __( "If you still don't see a green padlock or notice <b>mixed content</b> warning in your browser console - please enable the below option to force HTTPS on all resources of site.", 'wp-letsencrypt-ssl' ) ) . ' ' . sprintf( __( "If you still notice mixed content issues or issues with browser padlock not showing on your site, please consider upgrading to PRO version and avail %sMixed Content Scanner%s which allows you to scan and identify exact issues causing browser padlock to not show!.", "wp-letsencrypt-ssl" ), '<strong><a href="https://wpencryption.com/mixed-content-scanner/?utm_campaign=wpencryption&utm_source=wordpress&utm_medium=mixedscanner" target="_blank">', '</a></strong>' ) . "</p>";
      ...
      <label>' . esc_html__( 'Force SSL via HTACCESS (Server level redirect - Faster)', 'wp-letsencrypt-ssl' ) . ' - ' . esc_html__( 'Most suitable for new sites', 'wp-letsencrypt-ssl' ) . '</label><br /><br />
    ...
    public function wple_force_https_handler()
    {
        ...
        <small>' . sprintf( esc_html__( 'In case you break the site, here is revert back to HTTP:// instructions - %s', 'wp-letsencrypt-ssl' ), 'https://wordpress.org/support/topic/locked-out-unable-to-access-site-after-forcing-https-2/' ) . '</small>';
            ...
            if ( $leopts['force_ssl'] != 0 ) {
                ...
                if ( $prevforce == 2 ) {
                    //previously htaccess forced so remove them
                    $this->wple_clean_htaccess();
                }
                update_option( 'siteurl', str_ireplace( 'https:', 'http:', get_option( 'siteurl' ) ) );
                update_option( 'home', str_ireplace( 'https:', 'http:', get_option( 'home' ) ) );
    ...
    <h4>' . esc_html__( 'Does installing the plugin will instantly turn my site https?', 'wp-letsencrypt-ssl' ) . '</h4>
      ...
      <p>' . $this->wple_kses( sprintf( __( 'Please use our <a href="%s" target="%s">Plugin support forum</a>. <b>PRO</b> users can register free account & use priority support at gowebsmarty.in. More info - https://wpencryption.com', 'wp-letsencrypt-ssl' ), 'https://wordpress.org/support/plugin/wp-letsencrypt-ssl/', '_blank' ), 'a' ) . '</p>';
        $this->generate_page( $page );
    }
